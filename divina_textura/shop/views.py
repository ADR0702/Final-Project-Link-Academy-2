...
def delivery_returns_view(request):
    return render(request, 'delivery_returns.html')
...
def all_products(request):
    ...
    # filtrare după categorie (ManyToManyField)
    if category_slug:
        products = products.filter(categories__slug=category_slug)

    # filtrare după subcategorie (ForeignKey)
    if subcategory_slug:
        products = products.filter(subcategory__slug=subcategory_slug)

    categories = Category.objects.all()

    # dacă e selectată o categorie → iau doar subcategoriile legate de ea
    if category_slug:
        subcategories = Subcategories.objects.filter(parent_category__slug=category_slug)
    else:
